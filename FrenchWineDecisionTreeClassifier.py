...
# 使用训练的模型进行预测，为了方便，
# 案例直接把测试集里面的数据拿出来三条
data = [[13.52,3.17,2.72,23.5,97],[12.42,2.55,2.27,22,90],[13.76,1.53,2.7,19.5,132]]
print(data)#利用3个数据进行测试，即取3个数据作为模型的输入端
#model.predict(X_test[0:3])
predict = model.predict(data)
# print(model.predict(data))#输出测试的结果，即输出模型预测的结果
for i in range(0,3):
	if predict[i] == 'Zinfandel':
		predict[i] = '仙粉黛'
	if predict[i] == 'Syrah':
		predict[i] = '西拉'
	if predict[i] == 'Sauvignon':
		predict[i] = '赤霞珠'
print(predict)             #输出测试的结果，即输出模型预测的结果
